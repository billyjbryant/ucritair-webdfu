...
(function () {
  ...
  // Add μCritAir specific status codes
  dfuse.STATUS_errTARGET = 0x01;
  dfuse.STATUS_errFILE = 0x02;
  dfuse.STATUS_errWRITE = 0x03;
  dfuse.STATUS_errERASE = 0x04;
  dfuse.STATUS_errCHECK_ERASED = 0x05;
  dfuse.STATUS_errPROG = 0x06;
  dfuse.STATUS_errVERIFY = 0x07;
  dfuse.STATUS_errADDRESS = 0x08;
  dfuse.STATUS_errNOTDONE = 0x09;
  dfuse.STATUS_errFIRMWARE = 0x0a;
  dfuse.STATUS_errVENDOR = 0x0b;
  dfuse.STATUS_errUSBR = 0x0c;
  dfuse.STATUS_errPOR = 0x0d;
  dfuse.STATUS_errUNKNOWN = 0x0e;
  dfuse.STATUS_errSTALLEDPKT = 0x0f;
  ...
  dfuse.Device.prototype.dfuseCommand = async function (command, param, len) {
    ...
    if (status.status != dfu.STATUS_OK) {
      ...
      throw `Special DfuSe command ${commandName} failed: state=${
        status.state
      }, status=${status.status} (${dfuse.getStatusDescription(
        status.status
      )})`;
    }
  };

  // Add helper function to describe status codes
  dfuse.getStatusDescription = function (status) {
    switch (status) {
      case 0x00:
        return 'No error';
      case 0x01:
  ...
  // Add helper function to recover from error states
  dfuse.Device.prototype.abortToIdle = async function () {
    try {
      await this.abort();
      ...
      this.logWarning('Error during abort: ' + error);
      throw error;
    }
  };
  ...
  dfuse.Device.prototype.do_download = async function (
    ...
    try {
      ...
      this.logWarning('Erase failed, attempting to continue: ' + error);
    }
    ...
    while (bytes_sent < expected_size) {
      ...
      // Implement retry logic for more robust transfers
      while (retries < MAX_RETRIES && !success) {
        try {
          // For μCritAir devices, add a small delay between SET_ADDRESS and download
          await this.dfuseCommand(dfuse.SET_ADDRESS, address, 4);
          this.logDebug(`Set address to 0x${address.toString(16)}`);

          // Small delay to let device process address change
          ...
          if (dfu_status.status != dfu.STATUS_OK) {
            if (
              dfu_status.status === dfuse.STATUS_errSTALLEDPKT &&
              retries < MAX_RETRIES
            ) {
              this.logWarning(
                `Got stalled packet error (status 15), retrying... (${
                  retries + 1
                }/${MAX_RETRIES})`
              );
              retries++;
              ...
              throw `DFU DOWNLOAD failed state=${dfu_status.state}, status=${
                dfu_status.status
              } (${dfuse.getStatusDescription(dfu_status.status)})`;
            }
          }
          ...
          this.logWarning(
            `Transfer failed, retrying... (${retries + 1}/${MAX_RETRIES})`
          );
          retries++;
          ...
          try {
            ...
            this.logWarning('Could not reset device state: ' + resetError);
          }
